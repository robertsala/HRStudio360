...
export default function JobPostingsPanel({ onOpenStudioAI, onNotification }: JobPostingsPanelProps) {
  ...
  const createJobMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      onNotification?.({ type: 'success', message: 'Job posting created successfully!' });
      resetForm();
      setShowCreateForm(false);
    },
    onError: (error: any) => {
      onNotification?.({ type: 'error', message: error.message || 'Failed to create job posting' });
    }
  });
  ...
  const updateJobMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/jobs'] });
      queryClient.invalidateQueries({ queryKey: ['/api/careers/jobs'] });
      onNotification?.({ type: 'success', message: 'Job posting updated successfully!' });
      resetForm();
      setEditingJob(null);
    },
    onError: (error: any) => {
      onNotification?.({ type: 'error', message: error.message || 'Failed to update job posting' });
    }
  });
  ...
  const deleteJobMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/jobs'] });
      queryClient.invalidateQueries({ queryKey: ['/api/careers/jobs'] });
      onNotification?.({ type: 'success', message: 'Job posting deleted successfully!' });
    },
    onError: (error: any) => {
      onNotification?.({ type: 'error', message: error.message || 'Failed to delete job posting' });
    }
  });
          ...
          <div className="flex items-center gap-4">
            ...
            {onOpenStudioAI && (
              <button
                ...
                data-testid="button-open-studio-ai-jobs"
              >
